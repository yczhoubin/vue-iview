...
export default {
  ...
  mounted() {
    MP().then(BMap => {
      // 百度地图API功能
      // 创建Map实例
      var map = new BMap.Map("baiduMapPage", { enableMapClick: true });
      // 初始化地图,设置中心点坐标和地图级别
      ...
      map.addEventListener('click', function(e) {
        if(self.pickupOpen) {
          ...
          alert("选择的位置：" + e.point.lng + "," + e.point.lat);
        }
      ...
      // //文字提示
      // var label = new BMap.Label("广州西站", {
      //   offset: new BMap.Size(140, 10)
      // });
    ...
    pickup: function(BMap, map) {
      ...
      CustomControl.prototype.initialize = function(map) {
        ...
        button.onclick = function(e) {
          let baiduMap = document.getElementById('baiduMapPage');
          if(self.pickupOpen) {  // 已经开启的点击按钮后关闭
            self.pickupOpen = false;
            button.value = 'on';
            baiduMap.style.cursor = "pointer";
          } else {   //开启点击拾取坐标
            self.pickupOpen = true;
            button.value = 'off';
            baiduMap.style.cursor = "default";
          }
        };
